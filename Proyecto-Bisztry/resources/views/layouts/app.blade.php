        ...
        <aside class="sidebar">
            ...
            <nav class="sidebar-nav">
                <ul>
                    ...
                    {{-- ======================================================= --}}
                    {{--        INICIO DE LA MEJORA DE VISIBILIDAD             --}}
                    {{-- ======================================================= --}}
                    {{-- Usamos la directiva @role para mostrar este elemento del menú --}}
                    {{-- SOLO si el usuario autenticado tiene el rol 'Super-Admin' --}}
                    @role('Super-Admin')
            ...
            <div class="sidebar-footer" x-data="{ open: false }">
                <div x-show="open" @click.away="open = false" class="logout-menu" x-transition>
                    <a href="{{ route('profile.edit') }}" class="logout-link"><i class="fas fa-user-edit fa-fw"></i><span>Mi Perfil</span></a>
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="logout-link"><i class="fas fa-sign-out-alt fa-fw"></i><span>Cerrar Sesión</span></button>
                    </form>
                </div>
                <div @click="open = !open" class="user-info-clickable">
                    ...
                    <div class="user-details">
                        ...
                        {{-- Muestra el primer rol del usuario, o 'Usuario' si no tiene ninguno --}}
...
<style>
    .sidebar-footer { padding: 1rem; border-top: 1px solid var(--border-color); margin-top: auto; position: relative; }
    .user-info-clickable { display: flex; align-items: center; gap: 0.75rem; padding: 0.5rem; border-radius: var(--radius-md); cursor: pointer; transition: var(--transition-fast); }
    .user-info-clickable:hover { background-color: var(--bg-color); }
    .avatar { width: 40px; height: 40px; border-radius: 50%; background-color: var(--primary-color); color: var(--text-on-primary); display: flex; align-items: center; justify-content: center; font-weight: 600; flex-shrink: 0; }
    .user-details { flex-grow: 1; overflow: hidden; white-space: nowrap; }
    .user-details h4 { font-size: 0.875rem; font-weight: 600; margin: 0; }
    .user-details span { font-size: 0.75rem; color: var(--text-secondary); }
    .icon-chevron { margin-left: auto; transition: transform 0.3s ease; }
    .rotate-180 { transform: rotate(180deg); }
    .logout-menu { position: absolute; bottom: calc(100% + 0.5rem); left: 1rem; right: 1rem; background-color: var(--surface-color); border-radius: var(--radius-lg); box-shadow: var(--shadow-md); z-index: 10; padding: 0.5rem; border: 1px solid var(--border-color); }
    .logout-link { display: flex; align-items: center; gap: 0.75rem; padding: 0.75rem; border-radius: var(--radius-md); text-decoration: none; color: var(--text-primary); width: 100%; background: none; border: none; font-size: 0.875rem; cursor: pointer; }
    .logout-link:hover { background-color: var(--bg-color); color: var(--primary-color); }
    .logout-link i { color: var(--text-secondary); }
</style>
